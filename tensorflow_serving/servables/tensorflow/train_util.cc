...
Status VerifySignature(const SignatureDef& signature) {
  if (signature.method_name() != kPredictMethodName &&
    ...
    return errors::Internal(strings::StrCat(
        "Expected training signature method_name to be one of {",
...
// Validate a SignatureDef to make sure it's compatible with training, and
// if so, populate the input and output tensor names.
Status PreProcessTraining(const SignatureDef& signature,
                            const TrainRequest& request,
...
// Validate results and populate a TrainResponse.
// Tensors are serialized as specified.
Status PostProcessTrainingResult(
...
Status RunTrain(
    ...
    const internal::TrainResponseTensorSerializationOption option,
    Session* session, const TrainRequest& request,
    TrainResponse* response) {
  // Validate signatures.
  ...
  std::vector<string> output_tensor_names;
  std::vector<string> output_tensor_aliases;
  TF_RETURN_IF_ERROR(PreProcessTraining(signature, request, &input_tensors,
                                          &output_tensor_names,
                                          &output_tensor_aliases));
  std::vector<Tensor> outputs;
  RunMetadata run_metadata;
  //todo here the loaded model needs to be trained again to get loss
  TF_RETURN_IF_ERROR(session->Run(run_options, input_tensors, //todo
                                  output_tensor_names, {}, &outputs,
                                  &run_metadata));

  return PostProcessTrainingResult(output_tensor_aliases, outputs, option,
                                     response);
}
}  // namespace internal
...
Status RunTrain(const RunOptions& run_options,
  ...
  return internal::RunTrain(
      ...
      internal::TrainResponseTensorSerializationOption::kAsProtoField,
