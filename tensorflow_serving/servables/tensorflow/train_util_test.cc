...
class TrainImplTest : public ::testing::Test {
 public:
...
std::unique_ptr<ServerCore> TrainImplTest::saved_model_server_core_;
std::unique_ptr<ServerCore> TrainImplTest::saved_model_server_core_bad_model_;
std::unique_ptr<ServerCore>
    TrainImplTest::saved_model_server_core_counter_model_;

TEST_F(TrainImplTest, MissingOrEmptyModelSpec) {
  TrainRequest request;
  TrainResponse response;

  // Empty request is invalid.
  EXPECT_EQ(tensorflow::error::INVALID_ARGUMENT,
            CallTrain(GetServerCore(), request, &response).code());
...
TEST_F(TrainImplTest, EmptyInputList) {
  TrainRequest request;
  TrainResponse response;
...
TEST_F(TrainImplTest, InputTensorsDontMatchModelSpecInputs) {
  TrainRequest request;
  TrainResponse response;
  ...
  Status status = CallTrain(GetServerCore(), request, &response);
...
TEST_F(TrainImplTest, OutputFiltersDontMatchModelSpecOutputs) {
  TrainRequest request;
  TrainResponse response;
  ...
  Status status1 = CallTrain(GetServerCore(), request, &response);
  ...
  TF_EXPECT_OK(CallTrain(GetServerCore(), request, &response));
  request.add_output_filter(kOutputTensorKey);

  // Duplicate output filter specified.
  Status status2 = CallTrain(GetServerCore(), request, &response);
...
TEST_F(TrainImplTest, InputTensorsHaveWrongType) {
  TrainRequest request;
  TrainResponse response;
...
TEST_F(TrainImplTest, ModelMissingSignatures) {
  if (IsTensorflowServingOSS()) {
    return;
  }
  TrainRequest request;
  ...
  EXPECT_EQ(tensorflow::error::FAILED_PRECONDITION,
            CallTrain(GetServerCoreWithBadModel(),
                        request, &response).code());
}

TEST_F(TrainImplTest, TrainionSuccess) {
  TrainRequest request;
  TrainResponse response;
  ...
  TrainResponse expected_response;
...
TEST_F(TrainImplTest, TrainionWithNamedRegressionSignature) {
  TrainRequest request;
  TrainResponse response;
  ...
  TF_ASSERT_OK(CallTrain(GetServerCore(), request, &response));
  TensorProto output_tensor_proto;
...
// Test querying a model with a classification signature. Train calls work
// with train, classify, and regress signatures.
TEST_F(TrainImplTest, TrainingWithNamedClassificationSignature) {
  TrainRequest request;
  TrainResponse response;
...
// Test querying a counter model with signatures. Train calls work with
...
// Train calls with only inputs or outputs. State is not supported in
// TensorFlow Serving on most scalable or production hosting environments.
TEST_F(TrainImplTest, TrainingWithCustomizedSignatures) {
  TrainRequest request;
  TrainResponse response;
  ...
  TF_ASSERT_OK(CallTrain(GetServerCoreWithCounterModel(),
                           request, &response));

  TrainResponse expected_get_counter;
  *expected_get_counter.mutable_model_spec() = *model_spec;
  ...
  TrainResponse expected_incr_counter;
  *expected_incr_counter.mutable_model_spec() = *model_spec;
  ...
  TrainResponse expected_reset_counter;
  *expected_reset_counter.mutable_model_spec() = *model_spec;
  ...
  TrainResponse expected_incr_counter2;
  *expected_incr_counter2.mutable_model_spec() = *model_spec;
  ...
  TrainResponse expected_incr_counter_by;
