...
using tensorflow::serving::TensorflowTrainer;
  ...
  if (http_method == "POST" &&
    ...
    if (method == "classify") {
      ...
      status = ProcessPredictRequest(model_name, model_version, request_body,
        ...
        //mine will run in the train request
      status = ProcessTrainRequest(model_name, model_version, request_body,
                                     output);
//    } else if (method == "mine") {
//      status = ProcessMineRequest(model_name, model_version, request_body,
//                                     output);
    }
...
Status HttpRestApiHandler::ProcessTrainRequest(
    const absl::string_view model_name,
    const absl::optional<int64>& model_version,
    const absl::string_view request_body, string* output) {
  ...
  JsonTrainRequestFormat format;
  TF_RETURN_IF_ERROR(FillTrainRequestFromJson(
      request_body,
  ...
  //Todo: DGu predictor to trainer
  TF_RETURN_IF_ERROR(
      trainer_->Train(run_options_, core_, request, &response));
