...
Status FillTrainRequestFromJson(
        ...
        get_tensorinfo_map,
        TrainRequest* request, JsonTrainRequestFormat* format) {
  rapidjson::Document doc;
  *format = JsonTrainRequestFormat::kInvalid;
  ...
  if (itr_instances != doc.MemberEnd()) {
    ...
    *format = JsonTrainRequestFormat::kRow;
    ...
    *format = JsonTrainRequestFormat::kColumnar;
...
Status MakeJsonFromTensors(const ::google::protobuf::Map<string, TensorProto>& tensor_map,
                           JsonTrainRequestFormat format, string* json) {
  if (tensor_map.empty()) {
    return errors::InvalidArgument("Cannot convert empty tensor map to JSON");
  }

  switch (format) {
    case JsonTrainRequestFormat::kInvalid:
      return errors::InvalidArgument("Invalid request format");
    case JsonTrainRequestFormat::kRow:
      return MakeRowFormatJsonFromTensors(tensor_map, json);
    case JsonTrainRequestFormat::kColumnar:
      return MakeColumnarFormatJsonFromTensors(tensor_map, json);
  }
}
